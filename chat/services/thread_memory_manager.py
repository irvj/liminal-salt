...
class ThreadMemoryManager:
    ...
    def _build_chatbot_prompt(self, persona_display_name, existing_block,
        ...
        return (
            ...
            "  along the way, not only the latest exchanges. Recency doesn't equal\n"
            "  importance — an early moment that established something meaningful\n"
            "  stays as load-bearing as a late one.\n"
            "- DETAIL settles to the level of natural memory. Exact quotes, long\n"
            "  verbatim passages, verbose descriptions → the gist. The gist of every\n"
            "  significant topic stays.\n"
    ...
    def _build_roleplay_prompt(self, persona_display_name, existing_block,
        ...
        return (
            ...
            "- KEEP vivid anchors when they carry the scene: a line of dialogue that\n"
            "  turned things, a sensory detail that defined a place. Use them\n"
            "  sparingly — memory, not transcript.\n"
            "- USE character names (not \"the user\" and not the persona's raw name if\n"
            "  a character name is clear from context). Write in third-person\n"
